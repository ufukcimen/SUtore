...
import { ArrowLeft, Laptop, RefreshCcw } from "lucide-react";
      ...
      try {
        ...
        const nextMessage =
          error instanceof Error
            ? error.message
            : "We could not load laptops right now.";
          ...
          <div className="mt-6 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <h1 className="mt-4 text-4xl font-semibold tracking-tight text-brand-ink">
                Browse laptops from the live catalog.
              </h1>
            </div>
        ...
        <section className="mt-8">
          {isLoading ? (
            <div className="rounded-[2rem] border border-slate-200/80 bg-white/85 px-6 py-12 text-center shadow-[0_18px_45px_rgba(7,17,31,0.08)]">
              ...
              <p className="mt-4 text-lg font-semibold text-brand-ink">Loading laptops...</p>
          ...
          {!isLoading && errorMessage ? (
            <div className="rounded-[2rem] border border-rose-200 bg-rose-50 px-6 py-10 text-center text-rose-900 shadow-[0_18px_45px_rgba(7,17,31,0.08)]">
              <p className="text-lg font-semibold">We could not load laptops.</p>
            ...
            products.length > 0 ? (
              <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                {products.map((product) => (
                  <LaptopCard key={product.id ?? product.product_id ?? product.name} product={product} />
                ))}
              </div>
            ) : (
              <div className="rounded-[2rem] border border-slate-200/80 bg-white/85 px-6 py-10 text-center shadow-[0_18px_45px_rgba(7,17,31,0.08)]">
                <p className="text-lg font-semibold text-brand-ink">No laptops found.</p>
                <p className="mt-2 text-sm text-slate-600">
                  The backend request completed successfully but returned no laptop
                  products right now.
                </p>
              </div>
            )
